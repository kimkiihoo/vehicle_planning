...
class Path:
    def __init__(self):
        ...
        self.y = []
        self.yaw = []
        self.directions = []  # 布尔值列表，True 表示前进，False 表示倒车
...
def interpolate(ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, directions):
    ...
    directions[ind] = l > 0.0  # 布尔值：True 为前进，False 为倒车
...
def generate_local_course(L, lengths, mode, maxc, step_size):
    ...
    py = [0.0] * npoint
    pyaw = [0.0] * npoint
    directions = [False] * npoint  # 初始化为布尔值
    ind = 1

    directions[0] = lengths[0] > 0.0  # 初始方向为布尔值
...
def calc_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size):
    ...
    for path in paths:
        ...
        path.directions = [bool(d) for d in directions]  # 确保布尔值
